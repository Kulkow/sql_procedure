SELECT AAA.trading_date as date,
       AAA.bond_id,
       AVG(coalesce(AR.`bid`, null)) over w as aBid,
       AVG(coalesce(AR.`ask`, null)) over w as aAsk
FROM
    (
        SELECT
...
GROUP BY AAA.`bond_id`, AAA.`trading_date`
    WINDOW w AS (
        partition by AR.trading_date order by AR.bond_id
